...
# Usage: package-releases.sh <version>
#   Version argument should include leading 'v'.
#   Optionally set AGENTS and/or SCRIPTS env vars to limit what gets built.
#     AGENTS  : space or comma separated subset of: claude cursor qwen roo copilot auggie gemini windsurf codex kilocode opencode (default: all)
#     SCRIPTS : space or comma separated subset of: sh ps (default: both)
#   Examples:
#     AGENTS=cursor SCRIPTS=sh $0 v0.0.1
#     AGENTS="copilot,gemini" $0 v0.0.1
#     SCRIPTS=ps $0 v0.0.1
...
# Create and use .genreleases directory for all build artifacts like spec-kit
...
build_variant() {
  ...
  local base_dir="$GENRELEASES_DIR/gk-${agent}-package-${script}"
  echo "Building $agent ($script) package..."
  mkdir -p "$base_dir"
  
  # Copy base structure but filter scripts by variant
  SPEC_DIR="$base_dir/.goalify"
  mkdir -p "$SPEC_DIR"
  
  [[ -d memory ]] && { cp -r memory "$SPEC_DIR/"; echo "Copied memory -> .goalify"; }
  ...
  if [[ -d scripts ]]; then
    ...
    case $script in
      sh)
        [[ -d scripts/bash ]] && { cp -r scripts/bash "$SPEC_DIR/scripts/"; echo "Copied scripts/bash -> .goalify/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
      ps)
        [[ -d scripts/powershell ]] && { cp -r scripts/powershell "$SPEC_DIR/scripts/"; echo "Copied scripts/powershell -> .goalify/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
    esac
  ...
  [[ -d templates ]] && { mkdir -p "$SPEC_DIR/templates"; find templates -type f -not -path "templates/commands/*" -exec cp --parents {} "$SPEC_DIR"/ \; ; echo "Copied templates -> .goalify/templates"; }
  # Inject variant into plan-template.md within .goalify/templates if present
  local plan_tpl="$base_dir/.goalify/templates/plan-template.md"
  if [[ -f "$plan_tpl" ]]; then
    plan_norm=$(tr -d '\r' < "$plan_tpl")
    # Extract script command from YAML frontmatter
    script_command=$(printf '%s\n' "$plan_norm" | awk -v sv="$script" '/^[[:space:]]*'"$script"':[[:space:]]*/ {sub(/^[[:space:]]*'"$script"':[[:space:]]*/, ""); print; exit}')
    if [[ -n $script_command ]]; then
      # Always prefix with .goalify/ for plan usage
      script_command=".goalify/$script_command"
...
ALL_AGENTS=(claude cursor qwen roo copilot auggie gemini windsurf codex kilocode opencode)
ALL_SCRIPTS=(sh ps)
